...
class Story {
  ...
  show (idOrName, noHistory = false) {
    ...
    try {
      ...
      if (e.isGoto) {
        this.show(e.idOrName, e.noHistory);
        return;
      }
  ...
  /**
   * Immediately halts showing a passage, and shows the passage corresponding
   * to the ID or name passed to the function.
   *
   * @function goto
   * @param {string|number} idOrName - ID or name of the passage
   * @param {boolean} noHistory - if true, then this will not be recorded in the
